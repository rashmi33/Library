...
RSpec.describe IssuedHistoriesController, type: :controller do
  ...
  context 'POST #create' do
    ...
    context 'negative test' do
      ...
      it 'should not create a issued_history with invalid issue_type' do
        FactoryGirl.create(:issued_history)
        post :create, issued_history: { issue_type: 'ABC',date_of_issue: Faker::Date.forward(100), date_of_return: Faker::Date.forward(100), issued_no: Faker::Number.number(2) }, format: :json
        response.should have_http_status(:unprocessable_entity)
      end
